# This file is `run_sim_transmission_evolution.jl`.
# Purpose of this is to simulate transmission evolution in an environment that
# varies in time and space. Results plotted in figures 4 and S2.
#
# This code is meant to run many simulations in parallel. Run it with
# "julia -p # run_sim_transmission_evolution.jl", where # is the
...
@everywhere flist = [f for f=("var", "const", "const") for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere slist = [s for s=("const", "var", "const") for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere mlist = [m for m=("const", "const", "var") for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere TSlist = [TS for fsm=1:3 for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere offlist = Float64[off for fsm=1:3 for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere dlist = Float64[d for fsm=1:3 for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere hlist = Float64[h for fsm=1:3 for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere vlist = Float64[v for fsm=1:3 for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere rlist = [r for fsm=1:3 for d=(0.005, 0.05, 0.25, 0.5) for TS=(16000, 32000) for off=(0.125, 0.25, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere filelist = ["transmission_evolution/time scale = $(TSlist[i]), offset = $(TSlist[i]*offlist[i])/d = $(dlist[i])/f = $(flist[i]), s = $(slist[i]), m = $(mlist[i])/hinit = $(replace("$(hlist[i])", "." => "-")), vinit = $(replace("$(vlist[i])", "." => "-")), sim_$(rlist[i]).csv" for i=1:length(dlist)]
...
@everywhere function myfun(d::Float64, f::String, s::String, m::String, TS::Int, off::Float64, h::Float64, v::Float64, myfile::String)
	...
	writeData(simHostCntrl(initPop, d, fec, est, mort,
		...
		2*10^7, 	# simulation run time = 2*10^7 time steps
		20000, 0), 	# record every 20000th time step
		myfile, 20000, 0);
end
