# This file is `run_sim_more_state_M.jl`.
# Purpose of this is to simulate transmission evolution, when patches spend more
# time in State M than State P (62.5% vs. 37.5%). Results plotted in figure S3.
#
# This code is meant to run many simulations in parallel. Run it with
# "julia -p # run_sim_more_state_M.jl", where # is the
...
@everywhere filelist = ["unequal_states_62.5_percent_M/time scale = $(TSlist[i]), offset = $(TSlist[i]*offlist[i])/d = $(dlist[i])/f = $(flist[i]), s = $(slist[i]), m = $(mlist[i])/hinit = $(replace("$(hlist[i])", "." => "-")), vinit = $(replace("$(vlist[i])", "." => "-")), sim_$(rlist[i]).csv" for i=1:length(dlist)]
...
@everywhere function myfun(d::Float64, f::String, s::String, m::String, TS::Int, off::Float64, h::Float64, v::Float64, myfile::String)
	...
	if f == "var"
		fec = (patch::Int, symb::Int, t::Int) -> 0.5*ceil((mod(t + (TS*off)*(patch-1) + (TS/2)*(symb-1), TS)/(TS - 1)) - (3/8)) + 0.5;
	else
		fec = (patch::Int, symb::Int, t::Int) -> 1.0;
	end

	if s == "var"
		est = (patch::Int, symb::Int, t::Int) -> 0.5*ceil((mod(t + (TS*off)*(patch-1) + (TS/2)*(symb-1), TS)/(TS - 1)) - (3/8)) + 0.5;
	else
		est = (patch::Int, symb::Int, t::Int) -> 1.0;
	end

	if m == "var"
		mort = (patch::Int, symb::Int, t::Int) -> 0.5*(1 - ceil((mod(t + (TS*off)*(patch-1) + (TS/2)*(symb-1), TS)/(TS - 1)) - (3/8))) + 0.5;
	else
