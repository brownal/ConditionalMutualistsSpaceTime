# This file is `sim_host_control.jl`.
# This file holds the functions for simulating host transmission evolution.
using Distributions
...
function makeBirth(fec::Function)
    function birth(pop::Vector{host}, t::Int)
        fecundities = [fec(pop[i].patch, pop[i].symb, t) for i=1:length(pop)];
...
function makeEstablishment(est::Function, mort::Function)
    function establishment(pop::Vector{host}, newborn::host, t::Int)
        if rand() < est(newborn.patch, newborn.symb, t)
            deathProbs = [if (pop[i].patch == newborn.patch) mort(pop[i].patch, pop[i].symb, t) else 0 end for i=1:length(pop)];
...
# Function for producing a monomorphic (in transmission rates) population of hosts infected with different symbionts.
#  - Input:
#      - popData: array of the number of hosts in each patch infected with each type of symbiont. Rows correspond to patches and columns to symbiont type (first column = uninfected, second column = 1st symbiont type, etc.). E.g. [1 2 0; 0 0 3] specifies a population with 2 patches each containing 3 hosts. Patch 1 has 1 uninfected host and 2 hosts infected with symbiont 1. Patch 2 has 3 hosts infected with symbiont type 2.
...
function simHostCntrl(pop::Vector{host}, d::Float64, fec::Function, est::Function,
    mort::Function, mu::Float64, sdH::Float64, sdV::Float64, st::Float64, c::Int,
    ...
    simHistory = fill(host(0, 0, 0.0, 0.0), length(pop), Int(floor((tmax - burnin)/record)));
    ...
    for t=1:tmax
        ...
        # establishment/death
        pop = establishment(pop, newborn, t);
