# This file is `run_sim_more_state_P.jl`.
# Purpose of this is to simulate transmission evolution, when patches spend more
# time in State P than State M (62.5% vs. 37.5%). Results plotted in figure S4.
#
# This code is meant to run many simulations in parallel. Run it with
# "julia -p # run_sim_more_state_P.jl", where # is the
...
@everywhere flist = [f for f=("var", "const", "const") for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere slist = [s for s=("const", "var", "const") for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere mlist = [m for m=("const", "const", "var") for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere TSlist = [TS for fsm=1:3 for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere offlist = Float64[off for fsm=1:3 for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere dlist = Float64[d for fsm=1:3 for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere hlist = Float64[h for fsm=1:3 for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere vlist = Float64[v for fsm=1:3 for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere rlist = [r for fsm=1:3 for d=(0.005, 0.025, 0.05, 0.25, 0.5) for TS=(32000) for off=(0.125, 0.25, 0.375, 0.5, 1.0) for h=(0.0, 0.5, 1.0) for v=(0.0, 0.5, 1.0) for r=1:3]
@everywhere filelist = ["unequal_states_62.5_percent_P/time scale = $(TSlist[i]), offset = $(TSlist[i]*offlist[i])/d = $(dlist[i])/f = $(flist[i]), s = $(slist[i]), m = $(mlist[i])/hinit = $(replace("$(hlist[i])", "." => "-")), vinit = $(replace("$(vlist[i])", "." => "-")), sim_$(rlist[i]).csv" for i=1:length(dlist)]
...
@everywhere function myfun(d::Float64, f::String, s::String, m::String, TS::Int, off::Float64, h::Float64, v::Float64, myfile::String)
	...
	if f == "var"
		fec = (patch::Int, symb::Int, t::Int) -> 0.5*ceil((mod(t + (TS*off)*(patch-1) + (TS/2)*(symb-1), TS)/(TS - 1)) - (5/8)) + 0.5;
	else
		fec = (patch::Int, symb::Int, t::Int) -> 1.0;
	end

	if s == "var"
		est = (patch::Int, symb::Int, t::Int) -> 0.5*ceil((mod(t + (TS*off)*(patch-1) + (TS/2)*(symb-1), TS)/(TS - 1)) - (5/8)) + 0.5;
	else
		est = (patch::Int, symb::Int, t::Int) -> 1.0;
	end

	if m == "var"
		mort = (patch::Int, symb::Int, t::Int) -> 0.5*(1 - ceil((mod(t + (TS*off)*(patch-1) + (TS/2)*(symb-1), TS)/(TS - 1)) - (5/8))) + 0.5;
	else
	...
	writeData(simHostCntrl(initPop, d, fec, est, mort,
		...
		2000, 15201000), 	# record every 2000th time step for the last ~ 8 * 10^5 time steps
		myfile, 2000, 15201000);
end
